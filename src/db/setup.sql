...
create table boards(id bigserial primary key, name text, code text, grid_type text, board_shape text, size numeric[]);
insert into boards(id,name,code,grid_type,board_shape,size)
values
(1,'chess board','chess','squares','square','{8,8}');



create table movement (id bigserial primary key, name text, moves text[], attack_same_as_move boolean, attack_moves text[], can_jump boolean);
insert into movement(id,name,moves,attack_same_as_move,attack_moves,can_jump) 
values 
...
create table promotion (id bigserial primary key, promoted_to_piece integer, promotion_condition text, promotion_condition_code text);
insert into promotion(id,promoted_to_piece,promotion_condition,promotion_condition_code) values (1, 2, 'last row','lr'), (2, 6, 'not first move','nfm');




create table pieces (id bigserial primary key, name text, code text unique, img text, movement_id integer references movement(id), promotion_id integer references promotion(id), strength numeric);
...
alter table promotion add constraint promotion_pieces_fkey foreign key (promoted_to_piece) references pieces(id);
...
insert into bounding_box (bounding_box)
values
('{8,2}'),
('{4,2}');
...
insert into starting_positions (bounding_box_id,piece_code,spaces)
values
(1, 'p','{1,2,3,4,5,6,7,8}'),
(1,'r','{9,16}'),
(1,'n','{10,15}'),
...
insert into starting_positions_map (game_type_id,teams,starting_position_id)
values
(1,'{1,2}',1),
(1,'{1,2}',2),
(1,'{1,2}',3),
...
insert into game_types (name,code,board_id,starting_piece_positions_map,win_condition)
values
('chess','chess',1,1,'k'),
...
alter table starting_positions_map add constraint starting_positions_map_game_types_fkey foreign key (game_type_id) references game_types(id);
...
SELECT
      ...
      coalesce(
      (
        SELECT array_to_json(array_agg(to_json(x)))
        FROM (
          SELECT *
          FROM  starting_positions_map spm
...
from starting_positions_map spm
where spm.game_type_id = 1



select game_type_id, array_agg(starting_position_id) as sp
from starting_positions_map spm
where game_type_id = 1
group by game_type_id


select * from starting_positions_map spm;
select * from starting_positions sp;

select * 
from starting_positions_map spm join starting_positions sp on (spm.starting_position_id = sp.id)
where game_type_id = 1


select x.game_type_id, array_agg(x.piece_code_spaces) as piece_code_spaces, x.bounding_box_id
from
(select game_type_id, array[unnest(array_agg(to_jsonB(piece_code))),unnest(array_agg(to_jsonb(spaces)))] as piece_code_spaces
from starting_positions_map spm join starting_positions sp on (spm.starting_position_id = sp.id)
--where game_type_id = 1
group by game_type_id) x
group by x.game_type_id
